...
class _AdminManageStudentState extends State<AdminManageStudent> {
  ...
  void _showAddStudentDialog(BuildContext context) {
    showDialog(
      ...
      builder: (context) => const _StudentRegistrationDialog(),
    );
  }
}
...
class _StudentRegistrationDialogState extends State<_StudentRegistrationDialog> {
  ...
  List<DivisionModel> _availableDivisions = [];
  DivisionModel? _selectedDivision;

  bool _isLoadingData = true;
  bool _isSaving = false;

  final String _generatedId = 'STU-${DateTime.now().millisecondsSinceEpoch.toString().substring(5)}';
  ...
  Future<void> _onCourseChanged(CourseModel? course) async {
    ...
    // Fetch divisions that belong to the selected course
    ...
    final divs = snapshot.docs.map((doc) => DivisionModel.fromMap(doc.id, doc.data())).toList();
    if (!mounted) return;
    setState(() {
      _availableDivisions = divs;
    });
  }
  ...
  Future<void> _handleRegister() async {
    ...
    if (_selectedCourse == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please select a course'), backgroundColor: AppColors.priorityUrgent));
      return;
    }
    if (_selectedDivision == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please select a division'), backgroundColor: AppColors.priorityUrgent));
      return;
    }
    ...
    if (error != null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Student Registered Successfully!'), backgroundColor: AppColors.priorityLow));
    }
  }
              ...
              child: const Row(
                children: [
                  ...
                  Expanded(child: Column(
                    ...
                    children: [
                      Text('Student Registration', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.white)),
                      Text('Enroll a new student', style: TextStyle(fontSize: 13, color: Colors.white70)),
                    ]
                  ))
                ],
              ),
                    ...
                    children: [
                      Row(
                        children: [
                          ...
                          Expanded(child: _buildInfoChip('Role: Student', Icons.admin_panel_settings)),
                        ],
                      ),
                      ...
                      _buildTextField(_enrollmentCtrl, 'Enrollment Number', Icons.format_list_numbered, (v) => v!.isEmpty ? 'Required' : null),
                      const SizedBox(height: 16),
                      ...
                      DropdownButtonFormField<CourseModel>(
                        key: ValueKey(_selectedCourse?.id ?? 'course'),
                        initialValue: _selectedCourse,
                        decoration: _buildDropdownDecoration('Course', Icons.menu_book),
                        items: _allCourses.map((c) => DropdownMenuItem(value: c, child: Text('${c.code} - ${c.name}'))).toList(),
                        onChanged: _onCourseChanged,
                        validator: (v) => v == null ? 'Please select a course' : null,
                      ),
                      const SizedBox(height: 16),
                      DropdownButtonFormField<DivisionModel>(
                        key: ValueKey(_selectedDivision?.id ?? _availableDivisions.length.toString()),
                        initialValue: _selectedDivision,
                        decoration: _buildDropdownDecoration('Division', Icons.groups),
                        items: _availableDivisions.map((d) => DropdownMenuItem(value: d, child: Text('${d.code} - ${d.name}'))).toList(),
                        onChanged: _selectedCourse == null ? null : (val) => setState(() => _selectedDivision = val),
                  ...
                  Expanded(
                    ...
                    child: ElevatedButton(
                      ...
                      child: _isSaving
                          ...
                          : const Text('Register Student', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)),
                    ),
                  ),
                ],
              ),
